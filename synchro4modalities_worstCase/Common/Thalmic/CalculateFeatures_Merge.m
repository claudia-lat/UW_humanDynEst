function [Feature, ClockWindowed] = CalculateFeatures_4Jon(RawEMG, Clock, selectedFeature, WindowSize, WindowShift, Norm)  
...
IndexWindow = [1:WindowShift:size(RawEMG,1);(1:WindowShift:size(RawEMG,1))+WindowSize];
IndexWindow(:,IndexWindow(2,:) > size(RawEMG,1)) = [];
IndexWindow(:,end+1) = [IndexWindow(1,end)+WindowShift, size(RawEMG,1)];

ClockWindowed = Clock(IndexWindow(1, :), :);
...
switch Norm
    ...
    case 'Median'
        RawEMG = RawEMG./repmat(median(RawEMG),length(RawEMG),1);
    case 'Mean'
        RawEMG = RawEMG./repmat(mean(RawEMG),length(RawEMG),1);
    case 'Max'
        RawEMG = RawEMG./repmat(max(RawEMG),length(RawEMG),1);
   ...
   %%
    case 14 % frequency features
        ...
        Data = arrayfun(@(x) [zeros(zeroPad,size(RawEMG,2)); ...
            ...
            zeros(zeroPad,size(RawEMG,2))], 1:length(IndexWindow),'un',0);
