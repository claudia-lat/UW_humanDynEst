function [subjectDataTemp, segmentCropOutput, segmentManualOutput, ekfOutput, imuOutput, forceOutput, primitiveName, useInTraining, mocapOutput] = ...
    ...
    try
        ...
        switch currSubjInfo.datasetName
            case {'kulic2009_tro', 'segmentdatasource'}
                
                switch currSubjInfo.datasetName
                    case {'kulic2009_tro'}
                        [subjectDataTemp.dataEkf.Q, subjectDataTemp.dataEkf.time, subjectDataTemp.dataSegManual] ...
                            = dataSource_TRO2009(obj, currSubjInfo.filePath, currSubjInfo.filePathDataEkf, currSubjInfo.filepathDataSegManual, obj.motionSet, 0);
            
                     case {'segmentdatasource'}
                        [subjectDataTemp.dataEkf.Q, subjectDataTemp.dataEkf.time, subjectDataTemp.dataSegManual] ...
                            = dataSource_generic(obj, currSubjInfo.filePath, currSubjInfo.filePathDataEkf, currSubjInfo.filepathDataSegManual, obj.motionSet, 0);
                end
                
                ekfDQTemp = diff(subjectDataTemp.dataEkf.Q)/mean(diff(subjectDataTemp.dataEkf.time));
                ekfDQ = zeroMeanCovarSignal(ekfDQTemp([1 1:size(ekfDQTemp, 1)], :), 0, 0);                
                subjectDataTemp.dataEkf.dQ = ekfDQ;
                
                currSubjInfo.filepathDataSegManual = '';
                currSubjInfo.filepathDataEkf = '';
                subjectDataTemp.exerciseType = 'Generic';
                subjectDataTemp.datasetName = 'Generic';
                
                switch currSubjInfo.exerciseName
                    case 'Training10Min'
                        subjectDataTemp.dataSegCrop.timeStart = 0;
                        subjectDataTemp.dataSegCrop.timeEnd = 10*60;
                        
                    case 'Testing10Min'
                        subjectDataTemp.dataSegCrop.timeStart = subjectDataTemp.dataEkf.time(end) - 10*60;
                        subjectDataTemp.dataSegCrop.timeEnd = subjectDataTemp.dataEkf.time(end);
                        
                    otherwise
                        subjectDataTemp.dataSegCrop = [];
                end
...
%                 subjectDataTemp.load;
    ...
    try
        ...
        switch currSubjInfo.datasetName
            case {'squats_tuat_2011', 'squats_tuat_2015', 'doppel', 'taiso_ut_2009', 'kulic2009_tro', 'segmentdatasource'}
