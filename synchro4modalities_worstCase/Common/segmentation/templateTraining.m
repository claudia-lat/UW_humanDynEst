function [trainingData, dimReduct, classifier, classifierTrainingLabel, score, timeTaken] = ...
    ...
    rng('default'); % for repeatability, the RNG is set to be a constant value
...
%     fprintf('Training Metrics (Tested only on points used for training), using input primitive labels \n');
...
%     printAccuracyToConsole(score);
    ...
    fprintf('Training Metrics (Tested only on points used for training), without using primitive labels \n');
    score = accuracyAssess(classifierTrainingLabel, groundTruthLabel); % last one was 'dataSelect.patientSelection.fullExercistList' for a while
    printAccuracyToConsole(score);    
