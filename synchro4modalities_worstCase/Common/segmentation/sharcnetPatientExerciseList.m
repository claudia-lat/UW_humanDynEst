...
switch dataSet
    case {'Kulic2009_TRO', 'SegmentDataSource'}
        ...
        exerciseStrings = [exerciseStrings];
        exerciseInstanceList = cell(1, length(exerciseStrings));
        
        for i = 1:length(exerciseStrings)
...
exerciseList{1} = {'HAAO_STD', 'HAAL_STD', 'HEFO_STD', 'HFEO_STD', 'KEFO_SIT', 'KFEO_SIT', 'KHEF_STD', 'KHEF_SUP'};
% exerciseList{1} = {'HFEO_SUP', 'KEFO_SIT', 'KHEF_SUP', 'SQUA_STD', 'STSO_SIT'};
for i = 1
    exerciseInstanceList{i} = ones(size(exerciseList{i}))*0;
end
end
